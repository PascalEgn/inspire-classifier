...
TEST_ABSTRACT = (
    "We extend the pre-image representation of exposed points of the numerical range of"
    " a matrix to all extreme points. With that we characterize extreme points which"
    " are multiply generated, having at least two linearly independent pre-images, as"
    " the extreme points which are Hausdorff limits of flat boundary portions on"
    " numerical ranges of a sequence converging to the given matrix. These studies"
    " address the inverse numerical range map and the maximum-entropy inference map"
    " which are continuous functions on the numerical range except possibly at certain"
    " multiply generated extreme points. This work also allows us to describe closures"
    " of subsets of 3-by-3 matrices having the same shape of the numerical range."
)
...
@pytest.mark.usefixtures("_trained_pipeline")
def test_vocab(app):
    with open(path_for("data_itos"), "rb") as f:
        data_itos = pickle.load(f)
    # For performance when using mixed precision,
    # the vocabulary is always made of size a multiple of 8, potentially by
    # adding xxfake tokens.
    adjusted_max_vocab = (
